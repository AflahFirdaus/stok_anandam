...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc() : super(AuthInitial()) {
    on<LoginSubmitted>((event, emit) async {
      emit(AuthLoading());
      try {
        debugPrint('[AuthBloc] Attempting login with username: ${event.username}');
        ...
        if (token.isNotEmpty) {
          ...
          try {
            ...
            getIt<CurrentUserStore>().loadFromApi().timeout(
              const Duration(seconds: 2),
              onTimeout: () {
                debugPrint('AuthBloc: User info load is taking longer than 2s, proceeding in background.');
                return; 
              },
            ).catchError((e) {
